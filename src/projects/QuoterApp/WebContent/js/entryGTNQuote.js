...
$(document).ready(function(){
	...
	$('.pricing-checkbox').change(function(){
		...
		if (this.checked) {
			...
			var matchIndex=0;
			populateLicenseFee(checkedId,determiningValue,matchIndex);
			
			var determiningSubValue = 0;
			var matchSubscriptionIndex=0;
			populateSubscriptionFee(checkedId,determiningSubValue,matchSubscriptionIndex);
			
			var determiningTierValue = 0;
			var matchTierIndex=0;
			populateTieredPrice(checkedId,determiningTierValue,matchTierIndex);
				
			var determiningFreightValue=0;
			var matchTransPricingIndex=0;
			populateMinTransactionPricing(checkedId,determiningFreightValue,matchTransPricingIndex);
...
function populateMinTransactionPricing(checkedId,determiningFreightValue,matchTransPricingIndex)
{
...
if((checkedId == 73 || checkedId ==74 || checkedId ==75 || checkedId ==76 || checkedId ==77 || checkedId ==78 || checkedId ==79 || checkedId ==80 || checkedId ==81 || checkedId ==82) && (matchTransPricingIndex != 0) && annualPrice == 0)
	...
	$('#perTransactionMinimumPrice'+checkedId).html(numberWithCommas(determiningFreightValue*pricePerTrans));
...
else if(checkedId == 65 || checkedId ==67)
	{
	  ...
	  $('#perTransactionMinimumPrice'+checkedId).html(numberWithCommas(annualPrice*totalParcelCarriers));
	}
else{
$('#perTransactionMinimumPrice'+checkedId).html(numberWithCommas(annualPrice));
}

}

function populateTieredPrice(checkedId,determiningTierValue,matchTierIndex)
{
			...
			if(tierValue == 1)
				{
				   var maxTierPrice = parseInt($('#maxTierPrice'+matchTierIndex).val());
					$('#tieredPrice'+checkedId).html(numberWithCommas(feePerTransaction*maxTierPrice));
				}
			else{
				$('#tieredPrice'+checkedId).html(numberWithCommas(feePerTransaction*determiningTierValue));
			}
		}
	else if(checkedId==65 || checkedId==67)
		{
		 var totalParcelCarriers = parseInt($('#totalParcelCarriers').val());
		  ...
		  $('#tieredPrice'+checkedId).html(numberWithCommas(annualPricingFee*totalParcelCarriers));
...
function populateLicenseFee(checkedId,determiningValue,matchIndex)
{
	...
	//alert("determiningValue"+determiningValue);
	//alert("determiningValue nan"+isNaN(determiningValue));
	if((checkedId ==65 || checkedId == 67) )
		{
		//alert("inside");
			if(!(isNaN(determiningValue)) && (determiningValue!=0))
				{
					$('#licensePrice'+checkedId).html(numberWithCommas(licenseFee*determiningValue));
				}
			else{
				$('#licensePrice'+checkedId).html(0);
			}
		}
	else {
		//alert("here");
		//alert("licensefee"+licenseFee);
	$('#licensePrice'+checkedId).html(numberWithCommas(licenseFee));
	}
	
}

function populateSubscriptionFee(checkedId,determiningSubValue,matchSubscriptionIndex)
{
	$('.minTier'+checkedId).each(function(i) {
		...
		//alert("max sub value"+(parseInt($('#maxTier'+this.id).val())));
		if(minSubscriptionValue == 0 && ((parseInt($('#maxTier'+this.id).val())) == 0))
			{
	 ...
	 if(checkedId == 65 || checkedId ==67)
	{
	  ...
	  $('#subscriptionPrice'+checkedId).html(numberWithCommas(annualSubscriptionFee*totalParcelCarriers));
	}
	 else {
		 $('#subscriptionPrice'+checkedId).html(numberWithCommas(annualSubscriptionFee));
	 }
}
...
$('.btn-refresh').click(function() {
	//alert("inside");
			var checked=1;
			$('.pricing-checkbox:checkbox:checked').each(function ()
			{
			    	checked=checked+1;
			    	//alert("id is"+this.id);
			    	populateLicenseFee(this.id,0,0);
			    	populateMinTransactionPricing(this.id,0,0);
			    	populateSubscriptionFee(this.id,0,0);
			    	populateTieredPrice(this.id,0,0);
			});
...
function numberWithCommas(x) {
	if((x !=0) && !isNaN(x))
		{
		return parseInt(x).toFixed(3).replace(/\B(?=(\d{3})+(?!\d))/g, ",").concat(" USD");
		}
	else{			
		return '';
	}
